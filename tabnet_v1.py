...
if __name__ == '__main__':
    ...
    # 填充缺失值
    train_public_less = train_public_less.fillna(-1)
    train_internet_less = train_internet_less.fillna(-1)
    ...
    for train_index, valid_index in skf.split(X_train, y_train):
        ...
        clf.fit(X_more_internet, y_more_internet, eval_set=[(X_less, y_less)], eval_metric=['auc'])
        y_proba = clf.predict_proba(X_less)[:, 1]
        score = roc_auc_score(y_less, y_proba)
        scores.append(score)
    scores = np.array(scores)
